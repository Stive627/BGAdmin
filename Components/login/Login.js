...
import RecoverUI from './RecoverUI'
...
function Login({getAuthenticated}) {
  ...
  return (
    <div className='relative h-screen w-screen'>
      <div className='flex justify-center'>
        <div className={`w-5/6 lg:w-1/5 mt-20`}>
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
            <input name='email' type='email' autoComplete='email' onFocus={()=>setStatus({...status, faillure:false})} style={{borderColor:'rgba(0, 0, 0, 0.58)', outlineColor:'rgba(214, 15, 18, 1)'}} className='border w-full py-2 px-1 rounded-md'  value={admin.email} onChange={e => setAdmin({...admin, email:e.target.value})} placeholder='Username or email'/>
            <input name='password' type='password' autoComplete='password' onFocus={()=>setStatus({...status, faillure:false})} style={{borderColor:'rgba(0, 0, 0, 0.58)', outlineColor:'rgba(214, 15, 18, 1)'}} className='border w-full py-2 px-1 rounded-md'  value={admin.password} onChange={e => setAdmin({...admin, password:e.target.value})} placeholder='Password'/>
          ...
          <p className=' text-center mt-7'>Forgot password? <span><button onClick={()=>setRecover(true)} style={{color:'rgba(214, 15, 18, 1)'}} className=' underline'>Recover</button></span></p>
      ...
      <RecoverUI condition={recover} handleClose={handleClose}/>
    </div>
